...
{
    func retrieveSymbolsAndReferences(at path: URL,
                                      ...
                                      codebaseRootFolder: URL) async throws -> CodeFolder
    {
        ...
        let resultingSubFolders: [CodeFolder] = try await (subfolders ?? []).asyncMap
        {
            ...
            try await subfolder.retrieveSymbolsAndReferences(at: path.appending(path: subfolder.name),
                                                             from: server,
        ...
        let resultingFiles: [CodeFile] = try await (files ?? []).asyncMap
        {
            ...
            let fileUri = CodeFolder.fileURI(forFilePath: path.appending(path: file.name),
    ...
    private static func fileURI(forFilePath filePath: URL,
                                inRootFolder rootFolder: URL) -> String
    {
        rootFolder.appendingPathComponent(filePath.path()).absoluteString
    }
}
